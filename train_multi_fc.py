...
def train_loop(dataloader, model, loss_fn, optimizer):
    ...
    for batch, (X, y) in enumerate(dataloader):
        ...
        #print(pred[0])
        # y=y.unsqueeze(1)
        loss = loss_fn(pred1, y[0])+loss_fn(pred2, y[1])+loss_fn(pred3, y[2])+loss_fn(pred4, y[3])+loss_fn(pred5, y[4])+loss_fn(pred6, y[5])
...
def test_acc(dataloader, model, loss_fn):
    ...
    with torch.no_grad():
        for X, y in dataloader:
            X,y =X.to(device), y.to(device)
            pred1,pred2,pred3,pred4,pred5,pred6 = model(X)
            test_loss += loss_fn(pred1, y).item()+loss_fn(pred2, y).item()+loss_fn(pred3, y).item()+loss_fn(pred4, y).item()+loss_fn(pred5, y).item()+loss_fn(pred6, y).item()
...
for i in range(1,7):
    ...
    acc=test_acc(test_dataloader, model, loss_fn)
